...
def analyze_and_log_signal(engine: VolumeChannelEngine, df, params: dict, logger: logging.Logger):
    ...
    print("-" * 40)
    print(f"Breakout Signal: {prev['breakout_signal']}")
    print(f"Volume Delta:    {prev['volume_delta']:.0f}")
    print("-" * 40)
...
def open_position(exchange: Exchange, engine: VolumeChannelEngine, df,
    ...
    try:
        ...
        exchange.set_margin_mode(symbol, params.get('risk', {}).get('margin_mode', 'isolated'))
